...
async function capNhatToaDo() {
  ...
  const [rows] = await connection.execute('SELECT id_tram, dia_chi FROM tram where dia_chi IS NOT NULL AND (vi_do IS NULL OR kinh_do IS NULL)');
  ...
  for (const row of rows) {
    ...
    if (toado) {
      ...
      console.log(`❌ Không tìm thấy tọa độ cho: ${row.id_tram} - ${row.dia_chi}`);
    }
