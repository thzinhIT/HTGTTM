...
export function ModalDeletePrice(props) {
  const { open, setIsOpenModalDelete, refetch, dataDelete } = props; // Destructure props to get open and onOpenChange
  const handleClose = () => {
    setIsOpenModalDelete(false); // Close the modal
  };
  const fecthDeleteUser = async () => {
    const res = await fetch(
      "http://localhost:3000/api/auth/price/deletePrice",
      {
        method: "DELETE",
        headers: {
