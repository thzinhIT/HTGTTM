...
const cardSchema = z.object({
  the_id: z
    .string()
    .min(1, "ID thẻ không được để trống")
    .max(10, "ID thẻ không được dài quá 10 ký tự"),
  loai_the: z
    .string()
...
export function ModalCreateCard(props) {
  const { open, setIsOpenModalCreate, fecthCard } = props; // Destructure props to get open and onOpenChange
    ...
    <>
      <Dialog open={open} onOpenChange={setIsOpenModalCreate}>
        <DialogContent className="sm:max-w-[500px] max-h-[90vh] overflow-y-auto rounded-xl p-4">
          <DialogHeader>
            <DialogTitle>Thêm loại thẻ</DialogTitle>
            <DialogDescription>
              Nhập thông tin loại thẻ để thêm vào hệ thống.
            </DialogDescription>
          </DialogHeader>
          ...
          <form onSubmit={handleSubmit(onSubmit)} className="space-y-4 py-4">
            <div>
              ...
              <Input id="loai_the" {...register("the_id")} />
              {errors.loai_the && (
