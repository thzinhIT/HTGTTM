...
export function ModalCreateUser(props) {
  const { open, setIsOpenModalCreate, fecthdata } = props; // Destructure props to get open and onOpenChange
  const {
    register,
    handleSubmit,
    formState: { errors },
  ...
  const onSubmit = (data) => {
    const fecCreateUser = async () => {
      const res = await fetch("http://localhost:3000/api/auth/users/addUser", {
        method: "POST",
        headers: {
