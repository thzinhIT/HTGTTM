...
export function ModalUpdateTicket(props) {
  const { open, setIsOpenModalUpdate, dataUpdate, refetch } = props; // Destructure props to get open and onOpenChange
  ...
  const {
    ...
    defaultValues: {
      ten_ve: dataUpdate?.ten_ve || "",
      loai_xe: dataUpdate?.loai_xe || "",
      diem_tngo: dataUpdate?.diem_tngo || "",
      thoi_luong: dataUpdate?.thoi_luong || "",
      thoi_han: dataUpdate?.thoi_han || "",
      phi_phat_sinh: dataUpdate?.phi_phat_sinh || "",
      ghi_chu: dataUpdate?.ghi_chu || "",
    },
  });
        ...
        <DialogContent className="sm:max-w-[500px] max-h-[90vh] overflow-y-auto rounded-xl p-4">
          ...
          <form onSubmit={handleSubmit(onSubmit)} className="space-y-4 py-4">
            ...
            <div>
              ...
              <Select
                value={tenVe}
                onValueChange={(value) => setValue("ten_ve", value)}
              >
